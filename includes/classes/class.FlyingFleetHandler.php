    ...
    private function MissionCaseMIP ($FleetRow)
    {
        ...
        if ($FleetRow['fleet_start_time'] <= time())
        {
            if ($FleetRow['fleet_mess'] == 0)
            {
                ...
                $UserPlanet         = doquery("SELECT name, galaxy, system, planet FROM {{table}} WHERE id = '" . $FleetRow['fleet_owner'] . "'", 'planets',true);
                $name               = $UserPlanet['name'];
                $name_deffer        = $planet['name'];
                $message_vorlage    = 'Un ataque con misiles (' .$FleetRow['fleet_amount']. ') de ' .$name.' ['. $UserPlanet['galaxy'] .':'. $UserPlanet['system'] .':'. $UserPlanet['planet'].'] ';
                $message_vorlage   .= 'al planeta ' .$name_deffer. ' ['. $FleetRow['fleet_end_galaxy'] .':'. $FleetRow['fleet_end_system'] .':'. $FleetRow['fleet_end_planet'].'] ' .'<br><br>';
